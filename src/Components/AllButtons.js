...
const AllButtons = () => {
    ...
    const { pageCountArr, loading, currentPage, setCurrentPage, pageData } = useContext(AppContext)
    ...
    return (
        <div className="flex w-3/5 mx-auto gap-2 flex-wrap my-8">
            ...
            {pageData.length > 0 && <button className="btn btn-success" disabled={currentPage === 1} onClick={() => setCurrentPage((pre) => pre - 1)}>&#8592;</button>}
            ...
            {pageData.length > 0 && <button className="btn btn-success" disabled={pageCountArr.length === currentPage} onClick={() => setCurrentPage((pre) => pre + 1)}>&#8594;</button>}
        </div>
    );
};
